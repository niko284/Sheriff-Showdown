...
local useHookWithRefCallback = require("@ui/hooks/useHookWithRefCallback")
	...
	local resizeCanvas = useCallback(function(scrollLayout: UIGridStyleLayout, scrollingFrame: ScrollingFrame)
		...
		if uiPadding then
			absoluteContentSizeY = absoluteContentSizeY
				+ ((uiPadding.PaddingTop.Offset / scaleRatio.scale) + uiPadding.PaddingBottom.Offset / scaleRatio.scale)
			absoluteContentSizeX = absoluteContentSizeX
				+ (
					(uiPadding.PaddingLeft.Offset / scaleRatio.scale)
					+ (uiPadding.PaddingRight.Offset / scaleRatio.scale)
				)
		end
